...
var IzzNumberTest = (function () {
    ...
    IzzNumberTest.prototype["izz.number.validate returns true for positive numbers"] = function () {
        if (izz.number.validate(50) != true)
            failed("Failed on a positive numbers");
        if (izz.number.validate(1.1) != true)
            failed("Failed on a positive numbers");
    };
    IzzNumberTest.prototype["izz.number.validate returns true for negative numbers"] = function () {
        if (izz.number.validate(-50) != true)
            failed("Failed on a negative numbers");
        if (izz.number.validate(-1.23) != true)
            failed("Failed on a negative numbers");
    };
    IzzNumberTest.prototype["izz.number.validate returns true for zero"] = function () {
        if (izz.number.validate(0) != true)
            failed("Failed on 0");
        if (izz.number.validate(0.0) != true)
            failed("Failed on 0.0");
    };
    IzzNumberTest.prototype["izz.number.validate returns false for non-numbers"] = function () {
        var data = [
