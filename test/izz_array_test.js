...
let IzzArrayTest = class IzzArrayTest {
    "izz.array.validate returns true for valid arrays"() {
        if (izzNumberArray.validate([1, 2, 3]) != true)
            failed("Failed on a valid number array");
    }
    "izz.array.validate returns true for empty array"() {
        if (izzNumberArray.validate([]) != true)
            failed("Failed on a empty array");
    }
    "izz.array.validate returns false for non-arrays"() {
        let data = [
    ...
    "izz.array.validate returns false for invalid arrays"() {
        let data = [
            ["invalid", "here"],
            [true, true, false],
            [null, { one: "two" }],
